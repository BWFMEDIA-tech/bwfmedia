...
/* ============================================================
 * Stage snapshot — single-trip join load (perf refactor).
 * Replaces the old pattern of independent queries + per-row
 * postgres_changes subscriptions on join. Public, narrow columns.
 * ============================================================ */

import { z as _zSnap } from "zod";
...
export const getPlayArenaSnapshot = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const list = (tracksRes.data ?? []) as Array<{
      id: string;
      title: string;
      artist_name: string;
    ...
    for (const t of list) {
      votes[t.id] = { up: t.like_count, down: t.dislike_count, score: t.score };
    }
    ...
    return {
      stream: {
        ...
        roomName: stream.room_name as string,
        status: stream.status as string,
        mode: (stream.mode as string | null) ?? null,
        hostId: (stream.host_id as string | null) ?? null,
      },
      playing,
      queue,
      votes,
      audienceCount: countRes.count ?? 0,
    };
  });

export type PlayArenaSnapshot = NonNullable<
  Awaited<ReturnType<typeof getPlayArenaSnapshot>>
>;
