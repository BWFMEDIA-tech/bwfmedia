...
export const getSectionStats = createServerFn({ method: "POST" })
  ...
  .handler(async () => {
    ...
    const [
      ...
      sb.from("merch_commissions").select("commission_cents,status"),
      sb.from("merch_commissions").select("commission_cents,user_id,created_at").neq("status", "paid").order("created_at", { ascending: false }).limit(20),
    ...
    return {
      ...
      commissionsPaidCents: sum((commissionsAll.data ?? []).filter((r: any) => r.status === "paid"), "commission_cents"),
