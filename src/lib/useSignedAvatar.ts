...
import { signAvatarUrl } from "@/lib/signed-avatar.functions";
...
async function fetchSigned(originalUrl: string): Promise<string | null> {
  ...
  const promise = signAvatarUrl({ data: { url: originalUrl, expiresIn: TTL_SECONDS } })
    .then((res) => {
      if (res?.url) {
        cache.set(key, {
          url: res.url,
          expiresAt: now + (res.expiresIn ?? TTL_SECONDS) * 1000,
        });
        return res.url;
      }
      return null;
